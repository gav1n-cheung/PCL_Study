...
///*从范围图像中提取NARF特征
//
//本教程演示了如何从范围图像中提取NARF关键点位置的NARF描述符。可执行文件可以让我们从硬盘中加载点云数据（如果没有提供点云数据，则创建一个），提取点云上的兴趣点，然后在这些位置计算描述符。
//然后，可以在图像和3D查看器中可视化这些位置。
//*/
//
...
//    //首先创建NarfDesciptor对象，并为其提供输入数据（关键点索引和范围图像）。然后设置两个重要参数。
//    //支持大小：该大小确定从其计算描述符的区域大小，以及是否应该使用NARF描述符的旋转不变版本。
//    //我们创建输出点云并进行实际计算，最后，我们输出关键点的数量和提取的描述符的数量，这两个数字可能不相同。
//    //首先，描述符的计算可能会失败，因为范围图像中没有足够的点，或者是
//    //
//    // ------------------------------------------------------
