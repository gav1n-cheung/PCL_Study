...
/*
连接两个点云的点
在本教程中，我们将学习如何连接两个不同点云的点。这里施加的约束是两个数据集中的字段的类型和数量必须相等。我们还将学习如何连接两个不同点云的字段（例如维度等）。这里施加的约束是
两个数据集中的点数必须相等。
*/
#include <iostream>
  ...
  mainFun3 (int argc, char** argv)
{
  ...
  pcl::PointCloud<pcl::PointXYZ> cloud_a, cloud_b, cloud_c;//定义3个点云，两个输入点云，一个输出点云
  pcl::PointCloud<pcl::Normal> n_cloud_b;
  pcl::PointCloud<pcl::PointNormal> p_n_cloud_c;

  // Fill in the cloud data,填充点云数据
  cloud_a.width  = 5;
